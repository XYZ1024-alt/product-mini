...
/**
 * 自动识别并转换对象中的 cloud:// 为 https://
 * @param {Object} data 包含图片链接的任意对象或数组
 */
const convertCloudToHttp = async (data) => {
  // 把对象转成字符串，利用正则暴力找出所有的 cloud:// 链接
  let dataStr = JSON.stringify(data);
  const cloudReg = /cloud:\/\/[^"'\\]+/g;
  const cloudUrls = dataStr.match(cloudReg);

  // 如果没有云链接，直接返回原数据
  if (!cloudUrls || cloudUrls.length === 0) return data;
  ...
  try {
    // 批量去云端换取真实的 HTTPS 链接
    const res = await wx.cloud.getTempFileURL({ fileList: uniqueUrls });

    // 替换字符串里的旧链接
    res.fileList.forEach(item => {
      if (item.status === 0) {
        // 使用正则全局替换，防止一个链接出现多次
        const replaceReg = new RegExp(item.fileID.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g');
        dataStr = dataStr.replace(replaceReg, item.tempFileURL);
      }
    });
